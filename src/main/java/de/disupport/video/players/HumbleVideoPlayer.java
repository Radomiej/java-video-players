...
import de.disupport.video.helper.BackgroundThreadsService;
...
public class HumbleVideoPlayer {
    private Runnable endVideoListener;
    private NextFrameListener nextFrameListener;
    private String videoPath;

    private AtomicBoolean stopRequest = new AtomicBoolean();
    private AtomicBoolean playing = new AtomicBoolean();

    public HumbleVideoPlayer(String videoPath, NextFrameListener nextFrameListener) {
        this.nextFrameListener = nextFrameListener;
        this.videoPath = videoPath;
    }


    public void setEndVideoListener(Runnable endVideoListener) {
    ...
    public void playVideoAsynch() {
        if(playing.get()) {
            System.out.println("Video allredy playing");
            return;
        }
        playing.set(true);
        BackgroundThreadsService.Instance.runBackgroundTask(() -> {
            try {
                playVideo();
            ...
            System.out.println("playVideoAsynch end");
        });
    }
    ...
    public void playVideo() throws InterruptedException, IOException {
                ...
                do {
                    ...
                    if (picture.isComplete()) {
                        image = displayVideoAtCorrectTime(streamStartTime, picture,
                                converter, nextFrameListener, systemStartTime, systemTimeBase,
                                streamTimebase);
                    }
                    offset += bytesRead;
        ...
        // will appreciate it.
        demuxer.close();
        if (!stopRequest.get() && endVideoListener != null) endVideoListener.run();
    }
    ...
    private BufferedImage displayVideoAtCorrectTime(long streamStartTime,
                                                    ...
                                                    final NextFrameListener frameUpdateListener, long systemStartTime,
        ...
        frameUpdateListener.nextFrame(image);
        return image;
    }
}
